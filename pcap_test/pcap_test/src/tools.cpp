	...
	if (success == 0) {
		...
		while (temp != NULL) {
			if ((temp->ifa_addr) && (temp->ifa_addr->sa_family == AF_INET)) {
				if (strcmp(temp->ifa_name, if_name) == 0) {
					result = true;
					break;
				}
			}
...
int parse_url_parms(char *data, struct yuarel_param *params)
{
	...
	if (p)
	{
		...
		url_query = (char *) malloc(strlen(p) + 1);
		strcpy(url_query, p);
		
		p = (char *) strstr((const char *) url_query, " ");
		*p = 0;
		
		num_parms = yuarel_parse_query(url_query, '&', params, MAX_URL_PARAMS);
...
bool get_action(char *data, char *action)
{
	...
	int num_params = parse_url_parms((char *) data, (struct yuarel_param *) &params);
